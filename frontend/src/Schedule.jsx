...
const isScheduledLater = value => Boolean(value && new Date(value).getTime() > Date.now())
...
function MatchCard({match, onClick, onStart, onFinish, busy = false}) {
  ...
  return <article className="w-full overflow-hidden rounded-xl border bg-white shadow-sm transition hover:border-blue-400">
    ...
    {ready && !final && <div className={`grid gap-2 border-t bg-slate-50 p-2 ${match.status === 'completed' ? 'grid-cols-1' : 'grid-cols-2'}`}>
      {match.status !== 'completed' && <button type="button" onClick={onStart} disabled={busy || match.status === 'ongoing'} title={earlyStart ? 'Pertandingan boleh dimulai sebelum waktu jadwal' : 'Mulai pertandingan sekarang'} className={`rounded-lg px-2 py-2 text-[10px] font-bold ${match.status === 'ongoing' ? 'bg-emerald-600 text-white' : earlyStart ? 'border border-amber-300 bg-amber-50 text-amber-800 disabled:opacity-40' : 'border bg-white text-emerald-700 disabled:opacity-40'}`}><Play className="mr-1 inline" size={12}/>{match.status === 'ongoing' ? 'Berlangsung' : earlyStart ? 'Mulai Lebih Awal' : 'Mulai'}</button>}
...
function MatchModal({match, venues, close, save, initialStatus, volleyball = false}) {
  const [form, setForm] = useState({scheduled_at:localDateTime(match.scheduled_at), venue:match.venue || venues[0], duration_minutes:match.duration_minutes || 60, status:initialStatus || match.status || 'unscheduled', score_a:match.score_a ?? '', score_b:match.score_b ?? '', best_of_sets:match.best_of_sets || 3, set_scores:normalizeVolleyballSets(match.best_of_sets || 3, match.set_scores), winner_id:match.winner_id || '', notify:false})
  const [busy, setBusy] = useState(false)
  const setSummary = volleyballSummary(form.best_of_sets, form.set_scores)
  const submit = async () => {
    setBusy(true)
    try {
      await save({...form, duration_minutes:Number(form.duration_minutes), score_a:form.score_a === '' ? null : Number(form.score_a), score_b:form.score_b === '' ? null : Number(form.score_b), set_scores:volleyball && ['ongoing','completed'].includes(form.status) ? form.set_scores.map(set => ({score_a:set.score_a === '' ? null : Number(set.score_a), score_b:set.score_b === '' ? null : Number(set.score_b), completed:set.completed})) : undefined, best_of_sets:volleyball && ['ongoing','completed'].includes(form.status) ? Number(form.best_of_sets) : undefined, winner_id:form.winner_id ? Number(form.winner_id) : null, scheduled_at:form.scheduled_at || null})
      close()
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4"><div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-3xl bg-white p-6">
    ...
    {form.status === 'completed' && <p className="mt-5 rounded-xl bg-emerald-50 p-3 text-sm font-bold text-emerald-800">{volleyball ? `Tandai setiap set selesai sampai salah satu tim memenangkan ${setSummary.requiredWins} set.` : 'Isi skor akhir kedua peserta untuk menyelesaikan pertandingan.'}</p>}
    {!volleyball && <div className="mt-5 grid grid-cols-[1fr_90px] items-center gap-3"><b>{nameOf(match.participant_a)}</b><input className="input text-center" type="number" min="0" value={form.score_a} onChange={event => setForm({...form, score_a:event.target.value})}/><b>{nameOf(match.participant_b)}</b><input className="input text-center" type="number" min="0" value={form.score_b} onChange={event => setForm({...form, score_b:event.target.value})}/></div>}
    {volleyball && ['ongoing','completed'].includes(form.status) && <VolleyballScoreEditor bestOf={form.best_of_sets} onBestOfChange={value => setForm(current => ({...current,best_of_sets:value}))} scores={form.set_scores} onScoresChange={value => setForm(current => ({...current,set_scores:value}))} participantA={nameOf(match.participant_a)} participantB={nameOf(match.participant_b)} chooseFormat={!['ongoing','completed'].includes(match.status)} disabled={busy}/>}
    ...
    <button type="button" className="btn-primary mt-5 w-full py-3" disabled={busy || (volleyball && form.status === 'completed' && !setSummary.decided)} onClick={submit}><Send size={16}/>{busy ? 'Menyimpan...' : form.status === 'completed' ? 'Simpan Hasil Pertandingan' : initialStatus === 'ongoing' ? 'Mulai Pertandingan' : 'Simpan Perubahan'}</button>
  </div></div>
}
...
export function ScheduleManager() {
  ...
  const startMatch = async match => {
    if (isScheduledLater(match.scheduled_at) && !confirm(`Pertandingan ini dijadwalkan ${displayDate(match.scheduled_at)}.\n\nMulai lebih awal sekarang? Waktu jadwal tetap disimpan sebagai referensi.`)) return
    if (data.competition.scoring_mode === 'volleyball_sets') { setSelected({match, initialStatus:'ongoing'}); return }
  ...
  return <div>
    ...
    {!data.draw ? <div className="rounded-3xl bg-amber-50 p-8 font-bold text-amber-800">Buat drawing dan bagan terlebih dahulu sebelum menyusun jadwal.</div> : <div className="grid gap-7">
      ...
      <section><div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-[.16em] text-violet-700">Mode Manual</div><h2 className="mt-1 font-display text-2xl font-bold">Atur jadwal secara manual</h2><p className="mt-1 text-sm text-slate-500">Tarik kartu ke slot waktu atau klik pertandingan untuk mengedit detailnya. Pertandingan tetap dapat dimulai sebelum waktu jadwal jika kondisi lapangan memungkinkan.</p></div><label className="text-xs font-bold"><input className="mr-2" type="checkbox" checked={notifyDrag} onChange={event => setNotifyDrag(event.target.checked)}/>Notifikasi saat dipindah</label></div>
      ...
      {selected && <MatchModal match={selected.match} initialStatus={selected.initialStatus} venues={data.competition.venues} volleyball={data.competition.scoring_mode === 'volleyball_sets'} close={() => setSelected(null)} save={fields => update(selected.match, fields)}/>}
    </div>}
  </div>
}
