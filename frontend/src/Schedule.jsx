...
import { AlertTriangle, CalendarDays, Clock3, GripVertical, MapPin, Maximize2, Minimize2, Plus, RefreshCw, Send, Sparkles, Trash2, Trophy, Tv } from 'lucide-react'
...
export function ScheduleManager() {
  ...
  return <div>
    <div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-[.18em] text-blue-600">Match operations</div><h1 className="mt-2 font-display text-3xl font-bold">Jadwal Pertandingan</h1><p className="mt-2 text-sm text-slate-500">Pilih penjadwalan otomatis atau atur setiap pertandingan secara manual. Seluruh waktu menggunakan WIB.</p></div><div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row"><Link to={`/lomba/${data.competition.slug}/tv`} target="_blank" className="btn-dark justify-center"><Tv size={17}/>Buka TV Mode</Link><select className="input w-full sm:w-72" value={data.competition.id} onChange={event => load(event.target.value)}>{data.competitions.map(competition => <option key={competition.id} value={competition.id}>{competition.title}</option>)}</select></div></div>
...
export function TournamentTv() {
  ...
  const load = useCallback(() => api(`/competitions/${slug}/schedule`).then(value => { setData(value); setLastUpdated(new Date()); setError('') }).catch(requestError => setError(requestError.message)), [slug])
...
export function SchedulePublic() {
  ...
  return <section className="min-h-screen bg-slate-50 px-5 py-12"><div className="mx-auto max-w-6xl"><div className="flex flex-wrap items-center justify-between gap-3"><Link to={`/lomba/${slug}`} className="text-sm font-bold text-slate-500">← Kembali ke detail lomba</Link><Link to={`/lomba/${slug}/tv`} target="_blank" className="btn-dark"><Tv size={16}/>Buka TV Mode</Link></div><div className="mt-7"><div className="text-xs font-black uppercase tracking-[.18em] text-blue-600">Jadwal Resmi · WIB</div><h1 className="mt-2 font-display text-4xl font-bold">{data.competition.title}</h1><p className="mt-2 text-slate-500">Seluruh waktu menggunakan WIB. Jadwal dapat berubah dan pembaruan panitia dikirim ke ruang login peserta.</p></div>
